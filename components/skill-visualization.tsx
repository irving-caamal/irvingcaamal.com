...
import { Code, Database, Globe, ChevronRight, Layers, TestTube, Cloud } from "lucide-react"
        ...
        <div className="flex flex-wrap gap-3 mt-6">
          <Button
            ...
            className={`elegant-card border-white/10 transition-all duration-200 ${
              selectedCategory === null ? "bg-foreground/10 text-foreground" : "hover:bg-white/5"
            }`}
          >
            All Skills
          ...
          {Object.entries(skillCategories).map(([key, category]) => {
            ...
            return (
              <Button
                ...
                className={`elegant-card border-white/10 transition-all duration-200 ${
                  isSelected ? "bg-foreground/10 text-foreground" : "hover:bg-white/5"
                }`}
              >
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {filteredSkills.map((skill, index) => {
            ...
            return (
              <div
                key={skill.id}
                className={`group elegant-card rounded-xl p-6 transition-all duration-300 hover:bg-white/5 cursor-pointer border border-white/5 ${
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-3">
                        <Badge variant="outline" className="elegant-card border-white/20 text-xs font-medium">
                          {expertiseLevel}
                        </Badge>
                        <span className="text-xs text-muted-foreground">{skill.yearsOfExperience}+ years</span>
                      </div>
                    </div>
                  </div>
                  <ChevronRight
                ...
                return (
                  <>
                    ...
                    <div>
                      ...
                      <p className="text-muted-foreground">{filteredSkills.length} technologies in this domain</p>
                    </div>
                  </>
                )
              })()}
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              ...
              <div className="text-center">
                <div className="text-3xl font-bold mb-1">
                  <span className="bg-gradient-to-r from-foreground to-foreground/70 bg-clip-text text-transparent">
                    {Math.round(
                      filteredSkills.reduce((acc, skill) => acc + skill.yearsOfExperience, 0) / filteredSkills.length,
                    )}
                  </span>
                </div>
